...
/**
 * Classe que representa o algoritmo Sarsa Lambda para o jogo do ladrilho.
 * @author Alexandre
 */
public class SarsaLambda {
     ...
     */
    public SarsaLambda(ControleLadrilho jog) {
        this.jogo = jog;
    ...
    private void inicializarTraces(Map<String, Double> traces, String s){
        for(Integer i : getAcoesDisponiveis(s)){
            String strSA = s + ", " + i;
            if(!traces.containsKey(strSA)){
                traces.put(strSA, 0.0);
            }
        }
    }

    private void atualizarTrace(Map<String, Double> traces, String s, String a, double newValue){
        String strSA = s + ", " + a;
        if(traces.containsKey(strSA)){
            traces.put(strSA, newValue);
        }
        else{
            throw new IllegalArgumentException("Trace de par estado-ação não inicializado");
        }
    }

    private Double getTrace(Map<String, Double> traces, String s, String a){
        String strSA = s + ", " + a;
        if(traces.containsKey(strSA)){
            return traces.get(strSA);
        }
        else{
            throw new IllegalArgumentException("Trace de par estado-ação não inicializado");
        }
     ...
     */
    public Map<String, String> treinar(int qtdEpisodios, double alfa, double gama, double lambda){
        tabelaQ = new HashMap<String, TabelaValorEstadoAcao>();
        Map<String, Double> traces = new HashMap<String, Double>();
        double alfa2 = alfa;
        ...
        for(int i = 0; i < qtdEpisodios; i++){
            ...
            while(!jogo.isGameOver()){
                ...
                //calcula erro
                double erro = r + gama * qSLinha.getValor(aLinha) - qS.getValor(a);
                double newTraceSA = getTrace(traces, s, a) + 1;
                atualizarTrace(traces, s, a, newTraceSA);
                for(Integer aInt : getAcoesDisponiveis(s)){
                    String a2 = String.valueOf(aInt);
                    double novoValorQs = qS.getValor(a2) + alfa2 * erro * getTrace(traces, s, a2);
                    qS.inserirValor(a2, novoValorQs);
                    newTraceSA = gama + lambda * getTrace(traces, s, a2);
                    atualizarTrace(traces, s, a2, newTraceSA);
                }
                s = sLinha;
                a = aLinha;
                step++;
    ...
    public static void main(String[] args) {
        try {
            ...
            SarsaLambda qL = new SarsaLambda(jogo);
            qL.setEcho(true);
            qL.setEpisilon(episilon);
            
            //treino
            long timeStart = System.currentTimeMillis();
            Map<String, String> politicaGerada = qL.treinar(qtdEpisodios, alfa, gama, lambda);
